...
client.on('ready', () => {
	...
	mongoEconomy.connectDatabase("mongodb+srv://Kuromei:Pr321004@cluster0.ou5bm.mongodb.net/MikaSama?retryWrites=true&w=majority"); // this only needs to be called once!
...
client.on('message', async (msg, reaction, user) => {
	try {
		...
		var hasLevelUp = await mongoEconomy.attributeXp(msg.member.id, msg.guild.id, randomXp);

		//if (hasLevelUp) {
			//let member = await mongoEconomy.get
        //}

		if (msg.awaitReactions) {
			msg.channel.send(`${msg.author.name} reacted with ${msg.reaction}`)
		}
...
client.on('disconnect', async event => {
	...
	await mongoEconomy.disconnectDatabase();
	console.log("Level Database disconnected");
	process.exit(0);
});
